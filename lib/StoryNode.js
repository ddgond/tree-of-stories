	...
	generateChildFromSelection(index, genre = "cyoa") {
		...
		if (this.isRoot) {
			const prompt = JSON.stringify({
				genre: genre,
				storyTitle: this.prompts[index],
			})
			const systemPrompt = `You are the story-seeding module of a Choose-Your-Own-Adventure AI. You are tasked with creating the start of a story, but you must not finish it. You must tell it in second person, and you must not list choices. Your input is a JSON object and your output is the opening story page.`;
			return AiGeneration.text(prompt, systemPrompt, 1024).then((story) => {
			...
			return this.generateStorySummary().then((storySummary) => {
				...
				if (!this.parent.isRoot) {
					summaryAndStory = `${storySummary}\n\nRecent context: ${this.story.split('\n').filter((line) => line.length > 0).join(' ')}`;
				}
				const prompt = JSON.stringify({
					genre: this.genre,
					storyTitle: this.storyTitle,
					storySoFar: summaryAndStory,
					optionChosen: this.prompts[index],
					storyTropeToUseInNewPage: trope,
				});
				const systemPrompt = `You are the story-telling module of a Choose-Your-Own-Adventure AI. You are tasked with continuing a story, using the option chosen by the player, but you must not finish it. You must tell it in second person, and you must not list choices. Your input is a JSON object and your output is the next story page.`;
				return AiGeneration.text(prompt, systemPrompt, 1024).then((story) => {
					// Generate new prompts for the new story
