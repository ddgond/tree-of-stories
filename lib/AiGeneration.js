...
function promptOptions(story, summary, genre, optionCount=5) {
	...
	const input = `Prior summary:\n${summary}\n\nCurrent story:\n${story}\n\nThis is the generation format:\n${promptTypes.map((promptType, index) => `${index+1}) [${promptType} response]`).join('\n')}\n\nDo not include the text in [brackets]. Generate ${optionCount} text-adventure-style options in the corresponding response style on how to continue the story:\n1)`;
