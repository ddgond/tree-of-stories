...
const Statistics = {
  elapsedTime: new DerivedStatistic('Elapsed Time', () => `${Math.round((Date.now() - Statistics.elapsedTime._resetTime) / 1000)} seconds`),
  ...
  openAiEstimatedCost: new DerivedStatistic('OpenAI Estimated Cost', () => formatMoney(Statistics.openAiTotalTokens.get() * OPEN_AI_COST_PER_TOKEN)),
  openAiEstimatedCostPerRequest: new DerivedStatistic('OpenAI Estimated Cost Per Request', () => formatMoney(Statistics.openAiTokensPerRequest.get() * OPEN_AI_COST_PER_TOKEN)),
  openAiEstimatedCostPerHour: new DerivedStatistic('OpenAI Estimated Cost Per Hour', () => formatMoney(Statistics.openAiTokensPerHour.get() * OPEN_AI_COST_PER_TOKEN)),
